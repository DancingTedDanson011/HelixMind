...
export function runCheckpointBrowser(options: BrowserOptions): Promise<BrowserResult> {
  ...
  return new Promise<BrowserResult>((resolve) => {
    ...
    // NOTE: The caller (chat.ts openRewindBrowser) already saves and removes
    // all stdin data listeners before invoking this function. We do NOT touch
    // keypress listeners — that would cause double-restore on exit.
    ...
    function cleanup(): void {
      ...
      // NOTE: The caller (chat.ts) restores data listeners after this function returns.
      // We only clean up our own handleKey listener here.
      process.stdout.write('\x1b[2J\x1b[H');
    }
